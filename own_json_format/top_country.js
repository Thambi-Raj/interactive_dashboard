var landkm_population=[
  ...
  function high_low(name){
          if(name=="lower"){
             area_val["data"]=dat.slice(landkm_population.length-10,landkm_population.length).reverse();
             var head = area_val["data"].map((d)=> d[0]);
             area_val['x_axis_domain'] = head;
             document.querySelector('.on').classList.remove('on');
             document.querySelector('#'+name).classList.add('on');
          }
          else{
                  area_val["data"]=dat.slice(0,10);
                  var head = area_val["data"].map((d)=> d[0]);
                  area_val['x_axis_domain'] = head;
                  document.querySelector('.on').classList.remove('on');
                  document.querySelector('#'+name).classList.add('on');
          }
           document.querySelector('#root>#content>#group2>#pop_den>.legend_area').remove();
          var verti = new Vertical_bar('#root>#content>#group2>#pop_den', area_val);
  ...
  function params_click(name){
          ...
          dat=landkm_population.map(d=> {return [d.country,d.density,d.population,d.area]});
          area_val["data"]=dat.slice(0,10);
          var head = area_val["data"].map((d)=> d[0]);
          area_val['x_axis_domain'] = head;
          area_val['x_subgroup'] = ['density','population','area'];
          area_val['ends']={'area':'sqkm'}
          area_val["map"]=[{"density":['density']},{'population':['population']},{'area':['area']}];
          area_val["color"]=['#FED17A','#83F2CB','#9CD3FE']
          var po = { "Country": 0, "density": 1, "population": 2, "area": 3};
          area_val["index"]=po;
        //  area_val["is_need"]=true;
          var parent = '#root>#content>#group1>#populationChart';
          area_val["legend_class"]='legend_area';
          var l = document.querySelector('#root>#content>#group2>#pop_den>.legend_area')
          l?l.remove():'';
          document.querySelector('.on').classList.remove('on');
